...
export async function llmJudge(
  ...
  const result = await generateObject({
    ...
    schemaDescription: "Evaluation of an AI response",
    messages: [
      {
        role: "system",
...
Scoring criteria:
- 10: Response fully address the task using the tools results correctly.
- 7-9: Response is mostly correct with minor errors.
- 4-6: Response partially address the task.
- 1-3: Response is mostly incorrect and irrelevant.
`,
      },
      {
        role: "user",
...
Tools results provided: ${JSON.stringify(target.mockToolResults)}
